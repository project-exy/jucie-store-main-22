...
export const NavBasket = ({ onProceed }: NavBasketProps) => { 
  ...
  return (
    <div className="absolute top-16 right-4 flex flex-col rounded-lg backdrop-blur-md bg-black bg-opacity-50 p-4 z-50 max-h-96 w-80 border-black border-2 shadow-lg">
      {/* Wewnętrzny biały border */}
      <div className="relative border-2 border-white rounded-lg p-2">
            ...
            basket.map((i) => (
              ...
              >
                ...
                <div className="flex flex-col w-1/2">
                  <p className="text-white step-title">{i.product.name}</p>
        ...
        <div className="mt-2">
          ...
          <div className="mt-2">
            <Link
              to="/basket"
              onClick={onProceed}
              className="text-white rounded-3xl py-1 px-6 font-medium inline-block 
                  hover:bg-transparent hover:border-white hover:text-white duration-300 hover:border-2 border-2 border-purple-950 step-title"
            >
              Proceed to Basket
            </Link>
          </div>
